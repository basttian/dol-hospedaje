...
/**
 * \file    test/phpunit/HOSPEDAJEFunctionalTest.php
 * \ingroup hospedaje
 ...
 *
 * @package Testhospedaje
 */
class HOSPEDAJEFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modHOSPEDAJE')->click();
		} else {
			// Disable the module
			$this->byHref('modHOSPEDAJE')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/hospedaje/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('hospedaje/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/hospedaje/admin/about.php');
		$this->authenticate();
		return $this->assertContains('hospedaje/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('hospedajewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modHOSPEDAJE_HOSPEDAJETriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modHOSPEDAJE_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
