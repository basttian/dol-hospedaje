			...
			xhttp.onreadystatechange = function() {
                if (this.readyState === 4 && this.status === 200) {
                	...
                	foreach($invoice->lines as $row){
                	    if ($idline == $row->id) {
                	?>
    					var descp = "<?php echo dol_escape_js($langs->trans('Persons')); ?>";
...
print '<button type="button" class="calcbutton2" id="btn_people_total" onclick="Ctd(\'e\');">Ctd</button>';
...
var numline = 0;
		...
		function ValidateCtdPeople()
		{
    		...
    		if (isNaN(peopleNumber)) {
    			console.error('Error not a valid number :', peopleNumber);
    			return;
    		}
    		window.location.href = 'hospedaje_selected_people.php?idline='+numline+'&persons='+peopleNumber+'&place=<?php echo $place;?>';
    		//parent.$.colorbox.close();
		}
</script>
<?php 