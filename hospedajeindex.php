...
llxHeader("", $langs->trans("HOSPEDAJEArea"));

print load_fiche_titre($langs->trans("HOSPEDAJEArea"), '', 'hospedaje.png@hospedaje');
...
// Draft MyObject
if (! empty($conf->hospedaje->enabled)&& $user->rights->hospedaje->read )
{
	$langs->load("orders");
...
if (! empty($conf->hospedaje->enabled) && $user->rights->hospedaje->read)
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."hospedaje_myobject as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
