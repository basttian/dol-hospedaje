...
/**
 * \file    hospedaje/admin/setup.php
 * \ingroup hospedaje
 * \brief   HOSPEDAJE setup page.
 */
...
require_once '../lib/hospedaje.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "hospedaje@hospedaje"));
...
$arrayofparameters = array(
	...
	//'HOSPEDAJE_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'HOSPEDAJE_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'HOSPEDAJE_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/hospedaje/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'HOSPEDAJE_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'HOSPEDAJE_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = hospedajeAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "hospedaje@hospedaje");
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'HOSPEDAJE_'.strtoupper($myTmpObjectKey).'_ADDON';
